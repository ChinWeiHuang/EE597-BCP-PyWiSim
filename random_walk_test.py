...
from RandomWalk import RandomNode
import argparse

parser = argparse.ArgumentParser()
parser.add_argument("--scheduling", choices=["LIFO", "FIFO"], default="LIFO", help="Scheduling method")
parser.add_argument("--arrival_rate", type=float, default=1, help="Packet arrival rate at node A (packets/second)")
...
def main():
    ...
    runtime = int(n_pkt_total / arrival_rate)  # simulation runtime in seconds
    print_flag = False
    loss=0.02
    ...
    for nid, x, y in [('A',0,0), ('B',1,0.5), ('C',1,-0.5), ('D',2,0.5), ('E',2,-0.5), ('F',3,0.5), ('G',3,-0.5)]:
        net.add_node(RandomNode(nid, is_sink=(nid == sink), scheduling_method=scheduling_method), x, y)

    # --- schedule packet arrivals at node A ---
    pkt_list = ["pkt " + str(i) for i in range(n_pkt_total)]
    pkt_counter = 0
    for time in range(runtime):
        for idx_pkt in range(int(arrival_rate * time), int(arrival_rate * (time + 1))):
